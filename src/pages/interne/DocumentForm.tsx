...
import { getClients, getDepots, getProducts, nextCode, upsertDocument, getStock } from "@/store/localdb";
...
export default function InterneDocumentForm() {
  ...
  const save = () => {
    ...
    if (!clientId) return toast({ title: "Sélectionnez un client" });

    const id = `doc_${Date.now()}`;
    const code = nextCode("interne", "DV");
    const doc: Document = {
      id,
      code,
      type: "DV",
      ...
      clientId,
      lines,
      includeInAccounting: false, // Internal docs are NOT declared
    };
    upsertDocument(doc);
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center gap-2">
        <h1 className="text-xl font-semibold">Nouveau devis — Interne</h1>
      ...
      <Card className="border-amber-200 bg-amber-50/50">
        <CardContent className="py-3">
          <p className="text-sm text-amber-700">
            ...
            Ce document ne sera pas déclaré en comptabilité mais sera inclus dans les statistiques et affectera les stocks.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="flex justify-end">
        <Button onClick={save}>Enregistrer le devis interne</Button>
      </div>
    </div>
  );
}